...
import { PillGroupMulti, MyCustomPieChart } from '../components/SharedUI'; // 從 SharedUI 拿出來的共用元件

const AnalysisView = ({ records, currentRoom, setView, setViewingRecord }) => {
  ...
  }, [currentRoom?.accountDefaultRange]);
  ...
  const analysisFilteredRecords = records.filter(r => {
    ...
    if ((r.type || 'expense') !== analysisType) return false;
    ...
    if (analysisMenus.includes('method') && analysisSubSelections.method.length > 0) {
      ...
      if (['信用卡', '行動支付', '信用卡 / 行動支付', '銀行', '銀行 / 電子票證', '電子票證'].includes(r.method) && analysisSubSelections.subMethod.length > 0 && !analysisSubSelections.subMethod.includes(r.subMethod)) return false;
    }
  ...
  analysisFilteredRecords.forEach(r => {
    ...
    analysisGroupedData[key] = (analysisGroupedData[key] || 0) + r.amount;
  });
    ...
    <>
      ...
      <main className="scroll-container px-3 py-3 flex-1 overflow-y-auto pb-[90px] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] space-y-3">
          ...
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {uniqueRoles.map(role => (
                <button key={role} onClick={() => setAnalysisRoleFilter(role)} className={`px-3 py-1 rounded-lg text-[13px] font-bold transition-all duration-200 ${analysisRoleFilter === role ? 'bg-teal-500 text-white shadow-md' : 'bg-white text-gray-500 border border-gray-200 hover:bg-gray-50'}`}>{role}</button>
              ))}
            </div>
          </div>
          <div className="mb-3">
            ...
            <div className="flex bg-gray-50 rounded-xl p-1 border border-gray-100 shadow-inner">
              {['expense', 'income', 'transfer'].map(type => (
                <button key={type} onClick={() => {
                  setAnalysisType(type); setAnalysisMenus([]); 
                  setAnalysisSubSelections({ category: [], title: [], merchant: [], method: [], subMethod: [], payer: [] });
                }} className={`flex-1 py-1.5 rounded-lg text-[14px] font-extrabold transition-all duration-200 ${analysisType === type ? `${type === 'expense' ? 'bg-orange-400' : type === 'income' ? 'bg-green-500' : 'bg-blue-500'} text-white shadow-md transform scale-100` : 'text-gray-400 hover:text-gray-600 scale-95'}`}>
          ...
          <div className="flex flex-col gap-2 bg-white p-2 rounded-2xl shadow-sm border border-teal-100 mb-3">
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex shrink-0 gap-0.5 ml-0.5">
                <button onClick={() => { setAnalysisStartDate(getLocalLastMonthStartStr()); setAnalysisEndDate(getLocalLastMonthEndStr()); }} className={`px-2 py-1.5 rounded-lg text-[11px] font-bold transition-all ${(analysisStartDate === getLocalLastMonthStartStr() && analysisEndDate === getLocalLastMonthEndStr()) ? 'bg-teal-500 text-white shadow-sm' : 'bg-teal-50 text-teal-600 hover:bg-teal-100'}`}>上月</button>
                <button onClick={() => { setAnalysisStartDate(''); setAnalysisEndDate(getLocalTodayStr()); }} className={`px-2 py-1.5 rounded-lg text-[11px] font-bold transition-all ${analysisStartDate === '' ? 'bg-teal-500 text-white shadow-sm' : 'bg-teal-50 text-teal-600 hover:bg-teal-100'}`}>全部</button>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white p-3.5 rounded-[1.5rem] shadow-sm border-2 border-teal-50">
          <h2 className="font-bold text-teal-700 mb-1 text-[15px] flex items-center gap-2"><BarChart size={16} className="text-teal-400"/> 統計結果</h2>
                ...
                return analysisDetailRecords.map(exp => (
                  <div key={exp.id} onClick={() => setViewingRecord(exp)} className="bg-gray-50 p-2.5 rounded-xl border border-gray-100 flex justify-between items-center cursor-pointer hover:bg-gray-100 transition">
                    <div className="flex-1 pr-2">
                      <div className="text-[11px] font-bold text-gray-400 mb-0.5">{toROCYearStr(exp.date)} {exp.addedByRole && <span className={`${getRoleColorStyle(exp.addedByRole).text} ml-1`}>{exp.addedByRole}</span>}</div>
                      <div className="font-black text-black text-[14px]">{exp.type === 'transfer' ? '轉帳' : exp.title}</div>
                    </div>
                    <div className={`font-black text-[16px] shrink-0 ${exp.type === 'income' ? 'text-green-500' : 'text-gray-800'}`}>{exp.type === 'income' ? '+' : '-'}${exp.amount.toLocaleString()}</div>
                  </div>
                ));
              })()}
            </div>
